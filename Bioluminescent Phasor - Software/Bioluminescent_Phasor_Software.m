%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$%£$
%
%%  Program for processing-saving spectral phasor images:
%   Run the program (F5), select Dark and Bright calibration files from the folde
%   and then select the experiment file (multiple files that use the same calibration can
%   also be loaded in one run). The program smooths the data with a Gaussian filter
%   with sigma "GaussSmooth". Bright calibration files and phasor images are smoothed with a
%   median filter with size "MedFilt_Cal" and "MedFilt_Ph", respectively.

%   This program requires the "Signal Processing Toolbox" and the "Image
%   Processing Toolbox". 
%   Tested on MATLAB R2021B

%%  Output:
%   1 -   Phasor plot and polar histogram (saved as PNG)
%   2 -   Intensity and Fluorescence emission color images (saved as PNG)
%   3 -   Workspace with relevant data for each experiment file (saved as MAT)
...
GaussSmooth         = 1;            % Sigma for the Gaussian smoothing of intensity images
MedFilt_Cal         = 51;           % Size of the median filter for the Bright and Dark Calibrations
MedFilt_Ph          = 3;            % Size of the median filter for the phasor images
Nbins               = 128;          % Number of bins for the 2D phasor histogram
Range_PhasorPlot    = [-1 1];       % 
Phase_Range         = [0 2*pi];     % 

%% Define which channel belongs to which
SIN_camera = 2;                             % 2
SIN_filt   = 1; % 1 is left, 2 is right     % 1
COS_camera = 1;                             % 1
COS_filt   = 1; % 1 is left, 2 is right     % 1
if SIN_filt == 1, Str_SIN = 'left'; SIN_int = 2;  Str_INT_SIN = 'right'; else, Str_SIN = 'right'; SIN_int = 1;  Str_INT_SIN = 'left'; end
if COS_filt == 1, Str_COS = 'left'; COS_int = 2;  Str_INT_COS = 'right'; else, Str_COS = 'right'; COS_int = 1;  Str_INT_COS = 'left'; end

%% Select Calibration and Experiment Images
param_img = struct;
param_img.Fsin = [11.2 92.8]/100;
param_img.Fcos = [10.9 94.3]/100;
param_img.Lambda = [400 700];

%Dark Image
fprintf('Selecting Dark File...\n')
[FileName_Dark,PathName_Dark] = Select_Files('','Select Dark File');
%Bright Images
fprintf('Selecting Bright File...\n')
[FileName_Bright,PathName_Bright] = Select_Files('','Select Bright File');
%Sin-Cos-Int
fprintf('Selecting Experiment Files...\n')
[FileName_Experiments,PathName_Experiments] = Select_Files('','Select Experiment File(s)');

%% Alignment and Intensity Calibration
fprintf('Loading Calibration Datasets...\n')
img_Dark = double(MultiTiff2Stack_frompath(FileName_Dark{1},PathName_Dark));
img_Bright = double(MultiTiff2Stack_frompath(FileName_Bright{1},PathName_Bright));

% Separate image stacks from the two cameras for registration
n_img_avg = 10;
for i = 1:n_img_avg
    img_Exp = double(MultiTiff2Stack_frompath(FileName_Experiments{1},PathName_Experiments,2*i+[-1 0]));
    if i == 1
        img_sin_cam1 = img_Exp(:,:,SIN_camera);
        img_cos_cam2 = img_Exp(:,:,COS_camera);
    else
        img_sin_cam1 = img_sin_cam1 + img_Exp(:,:,SIN_camera);
        img_cos_cam2 = img_cos_cam2 + img_Exp(:,:,COS_camera);
    end
end

img_sin_cam1 = img_sin_cam1/n_img_avg;
img_cos_cam2 = img_cos_cam2/n_img_avg;

% Selecting cameras subregions for registration
pos0 = [128 256 330 900];
fprintf('Alignment...\n')
fprintf(['Select camera subregion...SIN Channel (',Str_SIN,')\n'])
[~,pos_Sin_Sin] = CropImage(img_sin_cam1,['Select SIN-SIN from image on the ',Str_SIN],pos0);
if iseven(pos_Sin_Sin(3)),pos_Sin_Sin(3)=pos_Sin_Sin(3)+1;end
if iseven(pos_Sin_Sin(4)),pos_Sin_Sin(4)=pos_Sin_Sin(4)+1;end
fprintf(['Select camera subregion...INT Channel (',Str_INT_SIN,')\n'])
[~,pos_Sin_Int] = CropImage(img_sin_cam1,['Select SIN-INT from image on the ',Str_INT_SIN,' (Drag, don''t resize)'],pos_Sin_Sin);
pos_Cos_Cos = pos_Sin_Sin;
pos_Cos_Int = pos_Sin_Int;

pos_all = [pos_Cos_Cos;pos_Cos_Int;pos_Sin_Sin;pos_Sin_Int];

pos_Cos_Cos = pos_all((COS_camera-1)*2+COS_filt,:);
pos_Cos_Int = pos_all((COS_camera-1)*2+COS_int,:);
pos_Sin_Sin = pos_all((SIN_camera-1)*2+SIN_filt,:);
pos_Sin_Int = pos_all((SIN_camera-1)*2+SIN_int,:);

Crop_Cos_Cos = (CropImage_Auto(img_cos_cam2,pos_Cos_Cos));
Crop_Sin_Sin = (CropImage_Auto(img_sin_cam1,pos_Sin_Sin));
Crop_Cos_Int = (CropImage_Auto(img_cos_cam2,pos_Cos_Int));
Crop_Sin_Int = (CropImage_Auto(img_sin_cam1,pos_Sin_Int));

%% Align SIN and INT - CAM1
fprintf('Aligning CAM1...')
CCF = xcorr2(Crop_Sin_Int-min(min(Crop_Sin_Int)),Crop_Sin_Sin-min(min(Crop_Sin_Sin)));
[r,c] = find(CCF==absmax(CCF));
[R,C] = size(Crop_Sin_Sin);
dr = round(R-r);
dc = round(C-c);
pos_Sin_Sin = pos_Sin_Sin + [dc dr 0 0];

%% Align COS and INT - CAM2
fprintf('Aligning CAM2...')
CCF = xcorr2(Crop_Cos_Int-mean(mean(Crop_Cos_Int)),Crop_Cos_Cos-mean(mean(Crop_Cos_Cos)));
[r,c] = find(CCF==absmax(CCF));
[R,C] = size(Crop_Cos_Cos);
dr = round(R-r);
dc = round(C-c);
pos_Cos_Cos = pos_Cos_Cos + [dc dr 0 0];

%% Align CAM1 - CAM2
fprintf('Aligning CAM1 and CAM2...\n')
CCF = xcorr2(Crop_Cos_Int-mean(mean(Crop_Cos_Int)),Crop_Sin_Int-mean(mean(Crop_Sin_Int)));
[r,c] = find(CCF==absmax(CCF));
[R,C] = size(Crop_Sin_Int);
dr = round(R-r);
dc = round(C-c);
pos_Sin_Int = pos_Sin_Int + [dc dr 0 0];
pos_Sin_Sin = pos_Sin_Sin + [dc dr 0 0];

param_img.pos_Cos_Cos           = pos_Cos_Cos;
param_img.pos_Sin_Sin           = pos_Sin_Sin;
param_img.pos_Cos_Int           = pos_Cos_Int;
param_img.pos_Sin_Int           = pos_Sin_Int;
param_img.R_INT_SIN             = Crop_Sin_Int./Crop_Sin_Sin;
param_img.R_INT_COS             = Crop_Cos_Int./Crop_Cos_Cos;

fprintf('Calculating Correction parameters...Dark\n')
Crop_Sin_Int_Dark = CropImage_Auto(mean(img_Dark(:,:,SIN_camera:2:end),3),param_img.pos_Sin_Int);
Crop_Sin_Sin_Dark = CropImage_Auto(mean(img_Dark(:,:,SIN_camera:2:end),3),param_img.pos_Sin_Sin);
Crop_Cos_Int_Dark = CropImage_Auto(mean(img_Dark(:,:,COS_camera:2:end),3),param_img.pos_Cos_Int);
Crop_Cos_Cos_Dark = CropImage_Auto(mean(img_Dark(:,:,COS_camera:2:end),3),param_img.pos_Cos_Cos);
fprintf('Calculating Correction parameters...Bright\n')
Crop_Sin_Int_Bright = CropImage_Auto(mean(img_Bright(:,:,SIN_camera:2:end),3),param_img.pos_Sin_Int);
Crop_Sin_Sin_Bright = CropImage_Auto(mean(img_Bright(:,:,SIN_camera:2:end),3),param_img.pos_Sin_Sin);
Crop_Cos_Int_Bright = CropImage_Auto(mean(img_Bright(:,:,COS_camera:2:end),3),param_img.pos_Cos_Int);
Crop_Cos_Cos_Bright = CropImage_Auto(mean(img_Bright(:,:,COS_camera:2:end),3),param_img.pos_Cos_Cos);
param_img.Sin_Int_Dark = medfilt2(check_inf_nan(Crop_Sin_Int_Dark),[MedFilt_Cal MedFilt_Cal],'symmetric');
param_img.Sin_Sin_Dark = medfilt2(check_inf_nan(Crop_Sin_Sin_Dark),[MedFilt_Cal MedFilt_Cal],'symmetric');
param_img.Cos_Int_Dark = medfilt2(check_inf_nan(Crop_Cos_Int_Dark),[MedFilt_Cal MedFilt_Cal],'symmetric');
param_img.Cos_Cos_Dark = medfilt2(check_inf_nan(Crop_Cos_Cos_Dark),[MedFilt_Cal MedFilt_Cal],'symmetric');
fprintf('Calculating Correction parameters...Scaling\n')
Crop_Cos_Cos = medfilt2(check_inf_nan(Crop_Cos_Cos_Bright),[MedFilt_Cal MedFilt_Cal],'symmetric') - param_img.Cos_Cos_Dark;
Crop_Cos_Int = medfilt2(check_inf_nan(Crop_Cos_Int_Bright),[MedFilt_Cal MedFilt_Cal],'symmetric') - param_img.Cos_Int_Dark;
Crop_Sin_Sin = medfilt2(check_inf_nan(Crop_Sin_Sin_Bright),[MedFilt_Cal MedFilt_Cal],'symmetric') - param_img.Sin_Sin_Dark;
Crop_Sin_Int = medfilt2(check_inf_nan(Crop_Sin_Int_Bright),[MedFilt_Cal MedFilt_Cal],'symmetric') - param_img.Sin_Int_Dark;
param_img.R_INT_SIN             = Crop_Sin_Int./Crop_Sin_Sin;
param_img.R_INT_COS             = Crop_Cos_Int./Crop_Cos_Cos;
%% Clear calibration variables

clearvars -except FLAG_solution GaussSmooth MedFilt_Ph Nbins Range_PhasorPlot bin_time Phase_Range PathName_Experiments FileName_Experiments param_img FileName_Dark PathName_Dark FileName_Bright PathName_Bright
...
%% Initialization
L_Exp           = length(FileName_Experiments);
Phasor_Plot     = cell(1);
G               = cell(1);
S               = cell(1);
Mod             = cell(1);
...
for i_exp = 1:L_Exp
    ...
    fprintf(['Processing Experiment...',num2str(i_exp),'/',num2str(L_Exp),'\nFrames left:'])
    fname = [PathName_Experiments,FileName_Experiments{i_exp}];
    info = imfinfo(fname);
    num_images = numel(info)/2;

    for i_img = 1:num_images
        fprintf([num2str(num_images-i_img),'-'])
        img_Exp = double(MultiTiff2Stack_frompath(FileName_Experiments{i_exp},PathName_Experiments,2*i_img+[-1 0]));
        if i_img == 1
            img_SIN_SIN = CropImage_Auto(img_Exp(:,:,2),param_img.pos_Sin_Sin);
            img_SIN_INT = CropImage_Auto(img_Exp(:,:,2),param_img.pos_Sin_Int);
            img_COS_COS = CropImage_Auto(img_Exp(:,:,1),param_img.pos_Cos_Cos);
            img_COS_INT = CropImage_Auto(img_Exp(:,:,1),param_img.pos_Cos_Int);
        else
            img_SIN_SIN(:,:,i_img) = CropImage_Auto(img_Exp(:,:,2),param_img.pos_Sin_Sin);
            img_SIN_INT(:,:,i_img) = CropImage_Auto(img_Exp(:,:,2),param_img.pos_Sin_Int);
            img_COS_COS(:,:,i_img) = CropImage_Auto(img_Exp(:,:,1),param_img.pos_Cos_Cos);
            img_COS_INT(:,:,i_img) = CropImage_Auto(img_Exp(:,:,1),param_img.pos_Cos_Int);
        end
    end
    fprintf('\n')
    fprintf('Smoothing...')
    if GaussSmooth~=0
        fprintf('SIN-SIN/')
        img_SIN_SIN = MovGaussAverage_Fourier_Stack(img_SIN_SIN,GaussSmooth);
        fprintf('SIN-INT/')
        img_SIN_INT = MovGaussAverage_Fourier_Stack(img_SIN_INT,GaussSmooth);
        fprintf('COS-COS/')
        img_COS_COS = MovGaussAverage_Fourier_Stack(img_COS_COS,GaussSmooth);
        fprintf('COS-INT')
        img_COS_INT = MovGaussAverage_Fourier_Stack(img_COS_INT,GaussSmooth);
    end
    fprintf('\n')

    filename_corr = FileName_Experiments{i_exp};
    pos = strfind(filename_corr,'.');
    filename_corr = filename_corr(1:pos(end)-1);
    pos_ = strfind(filename_corr,'_');
    filename_corr(pos_) = '-';

    fprintf('Summing all frames...\n')
    img_SIN_INT_bin = mean(img_SIN_INT,3) - param_img.Sin_Int_Dark;
    img_SIN_SIN_bin = (mean(img_SIN_SIN,3) - param_img.Sin_Sin_Dark).*param_img.R_INT_SIN;
    img_COS_INT_bin = mean(img_COS_INT,3) - param_img.Cos_Int_Dark;
    img_COS_COS_bin = (mean(img_COS_COS,3) - param_img.Cos_Cos_Dark).*param_img.R_INT_COS;

    fprintf('Fine Registration...\n')
    [img_SIN_INT_bin,img_SIN_SIN_bin,img_COS_INT_bin,img_COS_COS_bin] = Biolum_Registration_4ch(img_SIN_INT_bin,img_SIN_SIN_bin,img_COS_INT_bin,img_COS_COS_bin,400,'Auto');

    fprintf('Phasor Transformation...\n')
    G{i_exp}           = check_inf_nan(2*(img_COS_COS_bin-img_COS_INT_bin*param_img.Fcos(1))./(img_COS_INT_bin*(param_img.Fcos(2)-param_img.Fcos(1)))-1);
    S{i_exp}           = check_inf_nan(2*(img_SIN_SIN_bin-img_SIN_INT_bin*param_img.Fsin(1))./(img_SIN_INT_bin*(param_img.Fsin(2)-param_img.Fsin(1)))-1);
    Mod{i_exp}         = sqrt(G{i_exp}.^2+S{i_exp}.^2);
    Ph{i_exp}          = atan2_2pi(S{i_exp},G{i_exp});
    for i_bin = 1:size(G{i_exp},3)
        if i_bin == 1
            Phasor_Plot{i_exp} = HistShow(G{i_exp}(:,:,i_bin)+1i.*S{i_exp}(:,:,i_bin),Nbins,0,Range_PhasorPlot,1);
        else
            Phasor_Plot{i_exp}(:,:,i_bin) = HistShow(G{i_exp}(:,:,i_bin)+1i.*S{i_exp}(:,:,i_bin),Nbins,0,Range_PhasorPlot,1);
        end
    end
    fprintf('Displaying Results...\n')
    figure
    SpectralPhasor_FigureDisplay(mean(medfilt2(S{i_exp}(:,:,1),[1 1]*MedFilt_Ph,'symmetric'),3),mean(medfilt2(G{i_exp}(:,:,1),[1 1]*MedFilt_Ph,'symmetric'),3),Nbins,[400 700])
    title(filename_corr)
    saveas(gcf,[PathName_Experiments,filename_corr,'_Phasor.png'],'png')
    figure
    s1 = subplot(1,2,1);
    imagesc(img_SIN_INT_bin(:,:,1),[0 Inf])
    colorbar
    colormap(s1,hot)
    Figure_Format
    axis image
    title(filename_corr)
    s2 = subplot(1,2,2);
    imagesc(ColormapPlusInt(img_SIN_INT_bin(:,:,1),medfilt2(Ph{i_exp}(:,:,1),[1 1]*MedFilt_Ph,'symmetric'),Phase_Range,map))
    Figure_Format
    axis image
    figureFullScreen(gcf);
    title('Bioluminescence Emission Color')
    saveas(gcf,[PathName_Experiments,filename_corr,'_Images.png'],'png')
    close all
    G1 = G{i_exp};
    S1 = S{i_exp};
    fprintf('Saving Results - Single Dataset...\n')
    save([PathName_Experiments,FileName_Experiments{i_exp}(1:end-4),'_workspace.mat'],'G1','S1','img_SIN_INT_bin','img_SIN_SIN_bin','img_COS_INT_bin','img_COS_COS_bin','param_img')
end
...
function [A_thr,thr]=Threshold_User(A,thr0)

if nargin<2; [~,thr0] = Automatic_Threshold(A); end
...
while check==0
    ...
    Figure_Format
    axis square
    title('Enter new threshold or press ENTER')
    subplot(1,2,2)
    imagesc((A>thr0))
    Figure_Format
    axis square
    title(['thr=',num2str(thr0)])
    thr = input('Enter new threshold or press ENTER');
    if isempty(thr)
        thr = thr0;
        A_thr = A>thr;
        check = 1;
...
function SpectralPhasor_FigureDisplay(S1,G1,Nbins,Lambda_Range)
...
Range_PhasorPlot = [-1 1];
tmp                = HistShow(nonzeros(G1+1i*S1),Nbins,0,Range_PhasorPlot,1);

n_colors = Nbins;
tmprgb = ColormapPlusInt(ones(size(tmp)),tmp,[0 absmax(tmp)],jet(n_colors));
tmp1 = nonzeros(G1);
G_hist = histc(tmp1,linspace(-1,1,Nbins));
G_hist = G_hist/max(G_hist)*Nbins/10;
tmp1 = nonzeros(S1);
S_hist = histc(tmp1,linspace(-1,1,Nbins));
S_hist = S_hist/max(S_hist)*Nbins/10;

x1 = meshgrid((1:Nbins)-Nbins/2);
y1 = flipud(meshgrid((1:Nbins)-Nbins/2)');
TH = atan2_2pi(y1,x1);
R = sqrt(y1.^2+x1.^2);
TH_color = zeros(size(tmprgb));
colmap_spectrum = squeeze(spectrumRGB(linspace(Lambda_Range(1),Lambda_Range(2),n_colors)));
TH_vect = linspace(0,2*pi,n_colors+1);
for i = 1:n_colors
    logic_mask = TH>TH_vect(i)&TH<=TH_vect(i+1)&R<=Nbins/2;
    TH_color(:,:,1) = TH_color(:,:,1) + logic_mask*colmap_spectrum(i,1);
    TH_color(:,:,2) = TH_color(:,:,2) + logic_mask*colmap_spectrum(i,2);
    TH_color(:,:,3) = TH_color(:,:,3) + logic_mask*colmap_spectrum(i,3);
end
TH_color = TH_color/absmax(TH_color)*1;

logic_mask1 = repmat(tmp>(absmax(tmp)/1000),1,1,3);
TH_color(logic_mask1) = tmprgb(logic_mask1);

figure
subplot(1,2,2)
polarhistogram(nonzeros(atan2_2pi(S1,G1)),linspace(0,2*pi,Nbins),'edgealpha',0,'facealpha',1,'facecolor','r');
hold on
h = polarhistogram(nonzeros(atan2_2pi(S1,G1)),linspace(0,2*pi,Nbins),'linewidth',2);
h.DisplayStyle = 'stairs';
h.EdgeColor = 'k';
pax = gca;
pax.ThetaTick;
n = length(pax.ThetaTick);
tmpl = num2str(linspace(Lambda_Range(1),Lambda_Range(2),n));
tmpl = strsplit(tmpl,' ');
for i = 1:n
    tmpl{i} = [tmpl{i},'nm'];
end
pax.ThetaTickLabel = tmpl;

subplot(1,2,1)
imagesc(TH_color)
Figure_Format
axis image
viscircles(repmat(Nbins/2*[1 1],11,1),linspace(1,Nbins/2,11)','Color',[1 1 1]*0.5,'LineWidth',0.1);
hold on
bar(1:Nbins,-G_hist,'FaceColor','r','EdgeColor','r');
barh(Nbins:-1:1,-S_hist,'FaceColor','r','EdgeColor','r');
plot(1:Nbins,-G_hist,'k');
plot(-S_hist,Nbins:-1:1,'k');
plot(ones(1,Nbins)*Nbins,1:Nbins,'k');
plot(1:Nbins,ones(1,Nbins)*Nbins,'k');

figureFullScreen(gcf);

end
function SpectralPhasor_Simple(S1,G1,Nbins)
...
tmp                = HistShow(nonzeros(G1+1i*S1),Nbins,0,Range_PhasorPlot,1);
hold on
imagesc(flipud(tmp)), colormap(map_white)
plot_PhasorCircle_Spectral(Nbins/2,-Nbins/2)
...
function SpectralPhasor_Simple_noGrid(S1,G1,Nbins)
...
function M=MultiTiff2Stack_frompath(FileName,PathName,vect_images)

fname = [PathName,FileName];
info = imfinfo(fname);
if nargin<3
    vect_images = 1:numel(info);
end
...
function A=MovGaussAverage_Fourier_Stack(B,FWHM)
if FWHM>0
    Z = size(B,3);
    for i = 1:Z
        if i==1
            A = MovGaussAverage_Fourier(B(:,:,i),FWHM);
        else
            A(:,:,i) = MovGaussAverage_Fourier(B(:,:,i),FWHM);
        end
...
function A=MovGaussAverage_Fourier(B,FWHM)

if FWHM>0
    B(isnan(B))=0;
    B(isinf(B))=0;
    A=zeros(size(B));
    g=Gaussian2D_rect(size(B(:,:,1)),FWHM/2.35);
    F=fft2(g/sum(sum(g)));
    A=fftshift(ifft2(fft2(B).*F));
    A=real(A);
else
    A=B;
end
...
function g2=Gaussian2D_rect(SizeMatrix,sigma)

g2=zeros(SizeMatrix);
x_r=(1:1:SizeMatrix(1));
x_c=(1:1:SizeMatrix(2));
g_r=Gaussian(x_r,sigma,(SizeMatrix(1)+1)/2,1);
g_c=Gaussian(x_c,sigma,(SizeMatrix(2)+1)/2,1);

for i=1:1:SizeMatrix(2)
    g2(1:end,i)=g_r*g_c(i);
end
end
function A=mean2(B)
...
function fig_handle = figureFullScreen(fig_handle)
Pix_SS = get(0,'screensize');
fig_handle.Position = Pix_SS;
end

function H=HistShow(gs,Bin,Display,Range,NoBorder)
...
H=hist2d(gs,Bin,Bin,[Range(1) Range(2)],[Range(2) Range(1)]);
x=linspace(Range(1),Range(2),Bin);
y=linspace(Range(2),Range(1),Bin);
...
if Display==1
    Range_hist=[min(min(H)),max(max(H))];
    imagesc(x,y,H,Range_hist);
    set(gca,'Yscale','lin','Ydir','normal');
    set(gca,'Xscale','lin','Xdir','normal');
    xlabel('g')
    ylabel('s')
    Figure_Format_Graph
    colormap(map)
...
function [Crop,pos] = CropImage_Auto(Img,pos)

% for l = 1:size(Img,3)
% f = figure;
% imagesc(Img(:,:,l))
% h = images.roi.Rectangle(gca,'Position',pos,'StripeColor','r');
% pos = floor(h.Position);
% img_INT = h.createMask;
% if l == 1
% Crop = reshape(nonzeros(Img(:,:,l).*img_INT),[length(nonzeros(sum(img_INT,2))),length(nonzeros(sum(img_INT,1)))]);
% else
% Crop(:,:,l) = reshape(nonzeros(Img(:,:,l).*img_INT),[length(nonzeros(sum(img_INT,2))),length(nonzeros(sum(img_INT,1)))]);
% end
% close(f)
% end

Crop = Img(pos(2)+1:pos(2)+pos(4),pos(1)+1:pos(1)+pos(3),:);
end
function [Crop,pos] = CropImage(Img,Title,pos0)
...
title(Title)
colorbar
h = images.roi.Rectangle(gca,'Position',pos0,'StripeColor','r');
pause
pos = floor(h.Position);
img_INT = h.createMask;
Crop = reshape(Img(img_INT),[length(nonzeros(sum(img_INT,2))),length(nonzeros(sum(img_INT,1)))]);
close(f)
end
function Img=ColormapPlusInt(Img_int,Img_property,Range,CMap,Scale_Factor,outlier_option)
Img_property = check_inf_nan(Img_property);
if nargin<6
    outlier_option = 'Colormap';
end

l_map=length(CMap);
dRange=(Range(2)-Range(1))/l_map;
[X,Y]=size(Img_property);
Img=zeros([X Y 3]);

if strcmp(outlier_option,'White')
    CMap(1,:) = 1;
    CMap(end,:) = 1;
end
if strcmp(outlier_option,'Black')
    CMap(1,:) = 0;
    CMap(end,:) = 0;
end
if strcmp(outlier_option,'Black-White')
    CMap(1,:) = 0;
    CMap(end,:) = 1;
end
...
for i=1:X
    for j=1:Y
        ...
        CMap_tone=round((Img_property(i,j)-Range(1))/dRange);
        if (CMap_tone<=0),     CMap_tone = 1; end
        if (CMap_tone>=l_map), CMap_tone = l_map; end
        Img(i,j,:)=CMap(CMap_tone,:)*Img_int(i,j);
...
function [B,B1]=Soglia(A,soglia,soglia2,Display)
...
function [A_thr,thr]=Automatic_Threshold(A,N_std)

if nargin==1
    N_std=1;
end

thr=median(nonzeros(A))+std(nonzeros(A))*N_std;
A_thr=Soglia(A,thr,thr,0);

end
function [n, norm_val]= normalize_mine(f)
norm_val = absmin(f);
n = f-absmin(f);
norm_val(2) = absmax(n);
n = n/absmax(n);
end
function [img1_new,img2_new,img3_new,img4_new] = Biolum_Registration_4ch(img1,img2,img3,img4,min_size_thr,Mode)
if nargin<6, Mode = 'Manual'; end
if nargin<5, min_size_thr = 400; end
[optimizer, metric] = imregconfig('monomodal');
if strcmp(Mode, 'Manual')
    thr = Threshold_User(img1);
else
    thr = logical(Automatic_Threshold(img1));
end
cells = bwlabel(bwpropfilt(thr,'Area',[min_size_thr inf]));
check=0;
n_imgs = absmax(cells);
if strcmp(Mode, 'Manual')
    figure
    while check==0
        imagesc(bwareafilt(thr,n_imgs)), Figure_Format, axis image
        title(['N cells = ',num2str(n_imgs)])
        n_imgs_tmp = input('Insert number of cells or ENTER');
        if isempty(n_imgs_tmp)
            check=1;
        else
            n_imgs = n_imgs_tmp;
        end
...
thr = bwareafilt(thr,n_imgs);
cells = bwlabel(thr);
img1_new = zeros(size(img1));
img2_new = zeros(size(img2));
img3_new = zeros(size(img3));
img4_new = zeros(size(img4));
for i_cells = 1:n_imgs
    thr_tmp = cells==i_cells;
    [r,c] = find(thr_tmp);
    crop_r = [min(r) max(r)];
    crop_c = [min(c) max(c)];
    img1_tmp = img1(crop_r(1):crop_r(2),crop_c(1):crop_c(2));
    img2_tmp = img2(crop_r(1):crop_r(2),crop_c(1):crop_c(2));
    img3_tmp = img3(crop_r(1):crop_r(2),crop_c(1):crop_c(2));
    img4_tmp = img4(crop_r(1):crop_r(2),crop_c(1):crop_c(2));
    n1 = normalize_mine(img1_tmp);
    [n2, norm_val2] = normalize_mine(img2_tmp);
    [n3, norm_val3] = normalize_mine(img3_tmp);
    [n4, norm_val4] = normalize_mine(img4_tmp);
    %     movingreg = imregister(n2,n1,'translation',optimizer,metric);
    movingreg = imregister(n2,n1,'translation',optimizer,metric);
    img2_tmp1 = movingreg*norm_val2(2)+norm_val2(1);
    %     movingreg = imregister(n3,n1,'translation',optimizer,metric);
    movingreg = imregister(n3,n1,'translation',optimizer,metric);
    img3_tmp1 = movingreg*norm_val3(2)+norm_val3(1);
    %     movingreg = imregister(n4,n1,'translation',optimizer,metric);
    movingreg = imregister(n4,n1,'translation',optimizer,metric);
    img4_tmp1 = movingreg*norm_val4(2)+norm_val4(1);

    img1_new(crop_r(1):crop_r(2),crop_c(1):crop_c(2)) = img1_new(crop_r(1):crop_r(2),crop_c(1):crop_c(2))+img1_tmp.*thr_tmp(crop_r(1):crop_r(2),crop_c(1):crop_c(2));
    img2_new(crop_r(1):crop_r(2),crop_c(1):crop_c(2)) = img2_new(crop_r(1):crop_r(2),crop_c(1):crop_c(2))+img2_tmp1.*thr_tmp(crop_r(1):crop_r(2),crop_c(1):crop_c(2));
    img3_new(crop_r(1):crop_r(2),crop_c(1):crop_c(2)) = img3_new(crop_r(1):crop_r(2),crop_c(1):crop_c(2))+img3_tmp1.*thr_tmp(crop_r(1):crop_r(2),crop_c(1):crop_c(2));
    img4_new(crop_r(1):crop_r(2),crop_c(1):crop_c(2)) = img4_new(crop_r(1):crop_r(2),crop_c(1):crop_c(2))+img4_tmp1.*thr_tmp(crop_r(1):crop_r(2),crop_c(1):crop_c(2));
end

end
function cmap = map_white(n)
% Returns my custom colormap
if nargin==0
    n = 256;
end
...
function plot_PhasorCircle_Spectral(Ax_scale,Ax_shift)
...
for i = r
    xunit = i * cos(th);
    yunit = i * sin(th);
    plot(xunit*Ax_scale-Ax_shift, yunit*Ax_scale-Ax_shift,':','Color',[1 1 1]*0.7);
end
hold off
end
